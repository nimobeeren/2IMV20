...
If the entry and exit points are on \textbf{different sides} of the cutting plane, we first calculate the intersection of the ray with the cutting plane using the given provided method {\tt intersectFace}. Then, we render the volume using the rendering method from the side where the entry point is on, from the entry point to the intersection point.
...
\autoref{fig:backpack-mip} shows a backpack visualized using MIP. The items inside the backpack are visible and can be easily distinguished, because they have higher intensity than the surrounding material. As an example, this method could be useful to airport security.

\paragraph{Compositing}

The main advantage of compositing over MIP is the ability to visualize multiple layers of a volume. In particular, it is possible to highlight regions of different density by modifying the transfer function. Some of the differences between compositing and MIP can be seen in \autoref{fig:mipscomp}.
