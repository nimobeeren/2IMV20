...
A software raycaster is quite slow. To make the application more responsive we lower the resolution during rendering. The class Renderer already has a variable {\tt interactiveMode} that is set to true when there is interaction with the mouse, and is set to false when
interaction stops. We use this variable in function {\tt raycast()} in{\tt RaycastRenderer} class to check when interaction is present. This way we can lower the resolution by changing the values of the variables {\tt increment} and {\tt sampleStep} to bigget ones leading to lowering the number of steps along the view ray and the number of pixels sampled.  The new values for interactive mode were chosen using the trial-and-error method with respect to the best performance of the application. In \autoref{fig:speedup} can be observed both the time of the rendering in Interactive mode before and after the changes, and the resolution of the model.
...
Isosurfaces are defined by
 $$f(x; y; z) = C$$ 
Where C corresponds to the isovalue that defines the surface. Similarly to Ray Composite\textsuperscript{\autoref{ray_composite}}, we use the given {\tt rayVector} and {\tt sampleStep} values, we can calculate the incremental step to go from {\tt entryPoint} to {\tt exitPoint}. For each voxel, we use trilinear interpolation\textsuperscript{\autoref{trilinear_interpolation}} to calculate the value and then we compare it to the value of {\tt isoValueFront} which is the value specified in the GUI of the application. If the currenly value is greater we check if shading is enabled and apply it using {\tt computePhongShading} is needed, and return the the {\tt isoColorFront} which is the chosen color in the GUI of the application or the newly computer color after the shafing is applied to the  {\tt isoColorFront}. If no greater than {\tt isoValueFront} values are found black is returned. The result without shading enabled can be seen on \autoref{fig:isosurface}.
...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \caption{Interactive mode before speed up changes.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{after-speedup}
    \caption{Interactive mode after speed up changes.}
  \end{subfigure}
