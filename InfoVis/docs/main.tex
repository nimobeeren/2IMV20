...
We implemented a web application. Thus, the main technologies in use are HTML, CSS and JavaScript. For the interface, we decided to use Svetle\cite{svetle}, a relatively young framework designed to build web apps easily. Despite the existence of more mature frameworks, such as React, Vue.js and Angular, we decided to use Svetle as it is simple and enables us to be fast.
...
The main point of interaction of the user are the controls. The controls section is a set of dropdowns, buttons and other inputs. These allow the user to change the species and the view type. According to the selected view type, other options are shown.

For the yearly view, the user can select the start, end year, month, as well as the number of years per second - also known as the speed of the animation. These options affect: (1) the animation over the years, which can be started by pressing ``Start"; and (2) the base data used in the graph. In addition, while the animation is stopped, the user can choose a specific year to show in the maps.
