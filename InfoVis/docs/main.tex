...
The Terraclimate dataset \cite{terraclimatedata} consists of a large static muldimensional field containing items where each corresponds to a certain observation in a certain location and point in time. For each observation, a few attributes are provided, though we are only interested in the following subset:
...
\paragraph{Maximum temperature} A quantitive attribute represented as decimal degrees, in Celcius degrees.

\vspace{2mm}

Although other variables are available, such as the minimum temperature and precipitation, we chose to use the maximum temperature. By doing so, we can better visualize how the increasing temperatures have been influencing the dates of bird migrations.

% TODO: mention transformations
% note that the 2020 file might be released quite soon!
...
This visualization is designed to be used by a large variety of specialists and non-specialists alike. An ornithologist could study the migraiton of the Barn Swallow using the provided visualization. It is also suitabale for the needs of climate  change activists who wants prove a point about a real prolem or a curious person without specific domain knowledge who wants to see the effects of climate change on the nature. The tasks that  these people could undertake a described below.

\paragraph{Task 1: Effect of climate change on the migration of birds} One of the thing that can be achieve with this visualizaiton is showing the migraiton of birds with relation to the change in temperature in the past few years. A user can choose to see either a specific month and year from the available list or they could choose to period of time and the data will be displayed in an animation style. Questions that would be answered by this task is: What is the effect of climate change on the migration of birds?

\paragraph{Task 2: Patterns in bird migrations} Another thing that is possible to be visualized is the migration pattern of a specific type of bird over the course of a year. The breed of bird and the year of interest can be choosen from the available ones and a animation showing the data for each month for the chosen year will be visualized. Question that would be answered by this task is: What is the general migration pattern for bird X?

 \paragraph{Task 3: Specific birds responding to heat} Diffrent types of birds respond defferently to a change in temperatures. The tool can help visualize which types of birds are affected by the rising temperatures more than others  This can be done bby choosing all available types of birds or some specificaly chosen ones and comparing their migraiton behavior over the span of couple of years with relation to the temperatures during that time. Question that would be answered by this task is: Which types of birds respond to temperature change the most?
