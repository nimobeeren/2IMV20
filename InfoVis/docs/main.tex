...
For the second, we have chosen the GISTEMP dataset \cite{gistemp} by NASA, which provide the temperature anomalies - deviations from the corresponding 1951 to 1980 means - from 1880 up to 2020 in a regular 2° by 2° grid. The main advantage of this dataset relatively to others we found is that it uses the temperature anomalies. By using anomalies instead of absolute values \cite{gistempanomalies}, it is easier to see how colder or warmer a certain place is. These anomalies are relative to a 30-year period between 1951 and 1980.
...
The GISTEMP dataset \cite{gistemp} consists of a large static multidimensional field containing items where each corresponds to a certain observation in a certain location and point in time. For each observation, a few attributes are provided, though we are only interested in the following subset:
...
\paragraph{Temperature Anomaly} A quantitive attribute represented as decimal degrees, in Celcius degrees.
