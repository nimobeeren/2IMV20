...
In order to accurately visualize the problem, we need two different datasets - one that includes information about birds migration and one including temperature changes over the years. For the first needed dataset we have chosen the eBird dataset \cite{ebird2020data}. It includes all the information we need to calculate the start data of migration for different species of birds. The dataset is maintained by Cornell Lab of Ornithology, so every sample is reviewed carefully for accuracy of information. As different people contribute	to this dataset continuously, the information inside is up-to-date, but also keeps record of some years back.
