...
The Terraclimate dataset \cite{terraclimatedata} consists of a large static muldimensional table containing items where each corresponds to a certain observation in a certain location and point in time. For each observation, a few attributes are provided, though we are only interested in the following subset:

\paragraph{Latitude} A quantitative attribute represented as decimal degrees, varying from -90.0 to 90.0.

\paragraph{Longitude} A quantitative attribute represented as decimal degrees, varying from -180.0 to 180.0.
